    ...
    <div class="btc-footer__body">
      <div class="btc-footer__company">
        <div class="btc-footer__brand">
          <img src="/images/logo.png" alt="Beyond The Cloud" class="btc-footer__logo btc-footer__logo--dark" />
          <img src="/images/logo-dark.png" alt="Beyond The Cloud" class="btc-footer__logo btc-footer__logo--light" />
        ...
        <div class="btc-footer__badge-row btc-footer__desktop">
          <img src="/images/salesforcePartnerBadgeHorizontal.png" alt="Salesforce Partner" class="btc-footer__badge" />
          <div class="btc-footer__socials">
            <a v-for="s in data.social" :key="s.name" :href="s.href" :aria-label="s.name" class="btc-footer__social" target="_blank" rel="noopener noreferrer">
              <svg width="17" height="17" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path :d="icons[s.icon]"></path></svg>
            </a>
          </div>
        </div>
      </div>

      <div v-if="siteLinks.length" class="btc-footer__col btc-footer__col--site">
        <h4 class="btc-footer__label">This site</h4>
        <a v-for="l in siteLinks" :key="l.label" :href="l.href" class="btc-footer__link">{{ l.label }}</a>
      </div>
      <div class="btc-footer__col btc-footer__col--services">
        <h4 class="btc-footer__label">Services</h4>
        <a v-for="l in data.services" :key="l.label" :href="l.href" class="btc-footer__link">{{ l.label }}</a>
      </div>
      <div class="btc-footer__pair">
        <div class="btc-footer__col">
          <h4 class="btc-footer__label">Products</h4>
          <a v-for="l in data.products" :key="l.label" :href="l.href" class="btc-footer__link">{{ l.label }}</a>
        </div>
        <div class="btc-footer__col">
          <h4 class="btc-footer__label">Resources</h4>
          <a v-for="l in data.resources" :key="l.label" :href="l.href" class="btc-footer__link">{{ l.label }}</a>
        </div>
      </div>
      ...
      <div class="btc-footer__badge-row btc-footer__mobile">
        ...
        <div class="btc-footer__socials">
          <a v-for="s in data.social" :key="s.name" :href="s.href" :aria-label="s.name" class="btc-footer__social" target="_blank" rel="noopener noreferrer">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path :d="icons[s.icon]"></path></svg>
          </a>
        </div>
      </div>
    </div>
    ...
    <div class="btc-footer__legal">
      <div class="btc-footer__legal-inner">
        <div class="btc-footer__legal-company">
          <span>{{ data.company.name }}</span><span class="btc-footer__dot">·</span>
          <span>{{ data.company.nip }}</span><span class="btc-footer__dot">·</span>
          <span>{{ data.company.address }}</span><span class="btc-footer__dot">·</span>
          <a :href="'mailto:' + data.company.email" class="btc-footer__legal-link btc-footer__email">{{ data.company.email }}</a>
        </div>
        <div class="btc-footer__legal-links">
          <a :href="data.legal.privacyPolicy" class="btc-footer__legal-link">Privacy Policy</a><span class="btc-footer__dot">·</span>
          <a :href="data.legal.mainSite" class="btc-footer__legal-link">beyondthecloud.dev</a>
        </div>
...
html:not(.dark) .btc-footer {
  ...
  --f-tile-m: #f9f9f9;
  --f-logo-fx: drop-shadow(0 0 0.6px rgba(0, 0, 0, 0.7)) drop-shadow(0 1px 1.5px rgba(0, 0, 0, 0.35));
}
